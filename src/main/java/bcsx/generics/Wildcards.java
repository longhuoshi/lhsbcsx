 ...
 * 15.10.3 无界通配符
 * previous {@link UnboundedWildcards2}
 * next {@link  CaptureConversion}
 *
...
//        T t = holder.get();
    ...
    //而Holder<?>将持有具有某种具体类型的同构集合，因此不能只是向其中传递Object
...
//        holder.set(new Wildcards());//Same error
        ...
        //Can't do this ;don't have any 'T';a
//        T t = holder.get();
     ...
     * 这意味着如果T是Fruit，那么holder可以是Holder<Apple>，这是合法的。
     *为了防止将Orange放置到Holder<Apple>中，对set()的调用
...
//        holder.set(arg);// Error:
    ...
    /**
     * 与wildSubtype()相反：holder可以是持有任何T的基类型（父类型）的容器。
     * 因此，set()可以接受T，因为任何可以工作于基类的对象都可以多态地作用于导出类（这里就是T）。
     * 但是，尝试着调用get是没有用的。
...
//        T t = holder.get(); //Error:
    ...
    public static void main(String[] args) {
        ...
        //Unchecked conversion form Holder to Holder<T>
        //Unchecked method invocation:exact1(Holder<T>) is applied to (Holder,Long)
